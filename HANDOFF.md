...
- `src/engine/OverworldMap.*` + `src/engine/OverworldView.*`: multi-screen overworld runtime foundation. `OverworldMap` strictly loads/canonically writes format-2 `overworld/layout.json`, gives component screens stable IDs and spatial slots, derives cardinal seam reachability from facing cells that are walkable in the composed `Level`, composes separately authored definitions into one normal rules space, translates decorations and compound selector identities, injects the single map start, exposes global/local ownership and 3x3-neighborhood queries, fingerprints layout/content, and enforces production selector coverage plus the rule that one puzzle level's selectors live in one overworld screen. There are no authored connection records: ordinary gameplay rules decide whether a boundary move succeeds. Composition also accepts a complete in-memory layout/component override for editor drafts, including definitions with no file yet. `OverworldView` computes living-player action admission, fixed 3x3 framing, source/destination neighborhood union, and camera translation synchronized to player presentation. Production content now uses the composed path with stable screen ID 1; the legacy root-file loader remains only as a compatibility fallback. Covered by `tests/OverworldMapTests.cpp` (`sokoban_overworld_map_tests`).
...
- `src/engine/LevelEditor.*`: headless single-document editor model and command API. Owns document state/history, water-layer selection and layer-index maintenance, tile and mesh-decoration commands, decoration selection/transform validation, draft construction, level load/save, source/runtime mirroring, browser enumeration, puzzle screen/level renumbering, soft-delete/restore, and guarded permanent deletion. Layout-referenced overworld components are recognized by path rather than filename: their dimensions are locked, Player/End tiles are blocked, the start cell and its support are protected, puzzle-level selector ownership is enforced, saves validate the complete composed map transactionally, and draft play merges the unsaved component with the complete in-memory topology draft. Ordinary edge-tile edits open or close implicit seams. Map and layer insertion shift authored decoration coordinates in the same undoable transaction. It has no SDL, Vulkan, or ImGui dependency and is tested by `tests/LevelEditorTests.cpp` (`sokoban_level_editor_tests`).
- `src/engine/OverworldMapEditor.*`: headless project-level topology editor, separate from `LevelEditor` and shared by the ImGui panel and 3D interaction path. Owns the parsed layout/component draft, stable screen selection/identity, immediate neighboring-screen creation, screen moves, the start command and cell picker, soft delete/restore, undo/redo, complete draft overrides, and rollback-capable source/runtime save through `LevelProjectStore`. New screens are an unobstructed Ground floor with Air above, so new cardinal neighbors have an immediate implicit walkable seam. No connection records or connection commands exist; invalid unreachable drafts remain in memory but cannot replace files. Covered by `tests/OverworldMapEditorTests.cpp` (`sokoban_overworld_map_editor_tests`).
...
- `src/engine/LevelEditorDebugUi.*`: Debug-only ImGui adapter coordinating `LevelEditor` and the shared `OverworldMapEditor`. Its Overworld tab shows a scrollable spatial slot canvas with screen cards, directional `+N`/`+E`/`+S`/`+W` controls that immediately create all-Ground neighbors, start markers, selector counts, open/move/delete/restore controls, the click-driven start tool, topology undo/redo, implicit-seam guidance, and validated save. Unsaved topology overlays replace stale file-backed labels immediately. The adapter owns widget buffers and presentation only; every state transition and filesystem action is delegated to a headless model.
