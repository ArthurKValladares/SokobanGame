...
- `src/engine/OverworldMap.*` + `src/engine/OverworldView.*`: multi-screen overworld runtime foundation. `OverworldMap` strictly loads/canonically writes `overworld/layout.json`, gives component screens stable IDs and spatial slots, validates cardinal physical connections/reachability/undeclared seams, composes separately authored definitions into one normal `Level`, translates decorations and compound selector identities, injects the single map start, exposes global/local ownership and 3x3-neighborhood queries, fingerprints topology/content, and enforces production selector coverage plus the rule that one puzzle level's selectors live in one overworld screen. Composition also accepts a complete in-memory layout/component override for editor drafts, including definitions with no file yet. `OverworldView` computes living-player action admission, fixed 3x3 framing, source/destination neighborhood union, and camera translation synchronized to player presentation. Production content now uses the composed path with stable screen ID 1; the legacy root-file loader remains only as a compatibility fallback. Covered by `tests/OverworldMapTests.cpp` (`sokoban_overworld_map_tests`).
...
- `src/engine/LevelEditor.*`: headless single-document editor model and command API. Owns document state/history, water-layer selection and layer-index maintenance, tile and mesh-decoration commands, decoration selection/transform validation, draft construction, level load/save, source/runtime mirroring, browser enumeration, puzzle screen/level renumbering, soft-delete/restore, and guarded permanent deletion. Layout-referenced overworld components are recognized by path rather than filename: their dimensions are locked, Player/End tiles are blocked, start/connection endpoints and supports are protected, puzzle-level selector ownership is enforced, saves validate the complete composed map transactionally, and draft play merges the unsaved component with the complete in-memory topology draft. Map and layer insertion shift authored decoration coordinates in the same undoable transaction. It has no SDL, Vulkan, or ImGui dependency and is tested by `tests/LevelEditorTests.cpp` (`sokoban_level_editor_tests`).
- `src/engine/OverworldMapEditor.*`: headless project-level topology editor, separate from `LevelEditor` and shared by the ImGui panel and 3D interaction path. Owns the parsed layout/component draft, stable screen selection/identity, connected and disconnected creation, blocked connected moves, start/connection commands, explicit cell-picking state, soft delete/restore, undo/redo, complete draft overrides, and rollback-capable source/runtime save through `LevelProjectStore`. New connected screens begin with closed boundary walls except for the computed matching endpoint, so a one-command addition is immediately structurally valid. Invalid disconnected drafts remain in memory but cannot replace files. Covered by `tests/OverworldMapEditorTests.cpp` (`sokoban_overworld_map_editor_tests`).
...
- `src/engine/LevelEditorDebugUi.*`: Debug-only ImGui adapter coordinating `LevelEditor` and the shared `OverworldMapEditor`. Its Overworld tab shows a scrollable spatial slot canvas with screen cards, start markers, connection lines, selector counts, open/add/move/delete/restore controls, click-driven start/connection tools, connect/disconnect, topology undo/redo, and validated save. Picking opens the source component, highlights and labels the hovered occupancy cell in the 3D view, keeps the tool armed after invalid clicks, and supports explicit cancel. Unsaved topology overlays replace stale file-backed labels immediately. The adapter owns widget buffers and presentation only; every state transition and filesystem action is delegated to a headless model.
