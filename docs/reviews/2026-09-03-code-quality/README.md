...
**Maintainability follow-up, 2026-09-09 (shared test scaffolding):** The common test harness now supports labeled, scalar-near, and exception assertions; `ScopedTestDirectory` owns collision-checked temporary storage; and `TestAssetRoot` provides the single cross-platform `SOKOBAN_ASSETS` lookup. Fifteen migrated suites now use these utilities, including the filesystem-heavy content-pipeline coverage, removing local counters, assertion functions, scalar comparison functions, environment decoding, and timestamp/random directory implementations while preserving suite-specific diagnostics and fallback or skip behavior.
...
| Test assertions, asset roots, and temporary directories | `tests/TestHarness.hpp`; `tests/TestAssetRoot.hpp`; `tests/ScopedTestDirectory.hpp`; remaining older suites with local helpers | In progress 2026-09-09; fifteen suites now share assertions, asset-root lookup, and scoped temporary storage, with only the two Vulkan suites left to assess or migrate |
