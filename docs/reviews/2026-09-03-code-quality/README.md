...
**Maintainability follow-up, 2026-09-09 (shared test scaffolding):** The common test harness now supports labeled and scalar-near assertions, and `ScopedTestDirectory` provides collision-checked creation with best-effort scope cleanup without adding filesystem dependencies to assertion-only suites. Eleven migrated suites now use these shared utilities, including command-line, math, and geometry coverage, removing their local counters, assertion functions, scalar comparison functions, and timestamp/random temporary-directory implementations while preserving their diagnostic labels.
...
| Test assertions and temporary directories | `tests/TestHarness.hpp`; `tests/ScopedTestDirectory.hpp`; remaining older suites with local helpers | In progress 2026-09-09; eleven suites now share labeled assertions, scalar comparisons, and scoped temporary storage, with six older suites left to assess or migrate in coherent groups |
