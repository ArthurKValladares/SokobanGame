...
**Maintainability follow-up, 2026-09-09 (shared test scaffolding):** The common test harness now supports labeled, scalar-near, and exception assertions; `ScopedTestDirectory` owns collision-checked temporary storage; and `TestAssetRoot` provides the single cross-platform `SOKOBAN_ASSETS` lookup. Fourteen migrated suites now use these utilities, removing local counters, assertion functions, scalar comparison functions, environment decoding, and timestamp/random directory implementations while preserving each suite's fallback or skip behavior.
