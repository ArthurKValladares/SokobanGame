...
**Maintainability follow-up, 2026-09-09 (shared test scaffolding):** The common test harness now supports labeled, scalar-near, and exception assertions; `ScopedTestDirectory` owns collision-checked temporary storage; and `TestAssetRoot` provides the single cross-platform `SOKOBAN_ASSETS` lookup. Sixteen migrated suites now use these utilities, including filesystem-heavy content-pipeline and pure Vulkan device-selection coverage, removing local counters, assertion functions, scalar comparison functions, environment decoding, and timestamp/random directory implementations while preserving suite-specific diagnostics and fallback or skip behavior. `VulkanSmokeTests` remains a deliberate exception: it is one resource-lifecycle transaction with a top-level exception boundary and contains none of the duplicated assertion or temporary-directory scaffolding.
...
| Test assertions, asset roots, and temporary directories | `tests/TestHarness.hpp`; `tests/TestAssetRoot.hpp`; `tests/ScopedTestDirectory.hpp` | Completed 2026-09-09; sixteen suites migrated in coherent groups, and the standalone Vulkan smoke transaction was reviewed and retained because it has no duplicated scaffolding |
...
- Continue reducing the named `.clang-tidy` backlog in measured categories. `bugprone-empty-catch`, `performance-move-const-arg`, and `performance-unnecessary-value-param` were enabled on 2026-09-09 after resolving seven intentional exception barriers, 16 ineffective moves, and 14 read-only parameter copies with valid Clang 22 full-source scans. The earlier Clang 19 probe remains discarded; keep every other exclusion until the same clean-parse standard or the Linux gate verifies it.
...
The demonstrated CQ-01 through CQ-14 defects are resolved. The maintenance pass has also replaced ambiguous persistence booleans with revision-aware results, centralized repeated state-delta, texture-identity and puzzle-path rules, completed incremental PBR material import, enabled the Windows warnings-as-errors gate, removed stale tooling, replaced the chronological handoff with an authoritative guide while preserving its history, and migrated sixteen test suites to shared assertions, asset-root lookup and temporary-directory ownership. The current work documents intentional exception suppression, removes ineffective moves and read-only parameter copies, and enables `bugprone-empty-catch`, `performance-move-const-arg`, and `performance-unnecessary-value-param` after clean 131-translation-unit analyses.
