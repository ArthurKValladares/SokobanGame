...
// The board's grid overlay: whether it is drawn, and how.
//
// Lifted out of ApplicationDebugUi::draw, which was 854 lines - seven
...
void drawTileGridSection(const ApplicationDebugUi::Context& context)
{
...
// Per-tile-type sizes and the surface-entity footprint.
//
// Lifted out of ApplicationDebugUi::draw, which was 854 lines - seven
// independent panels and a preamble, sharing nothing but `context`. The
// CollapsingHeader call stays inside the function so the panel keeps its own
// open/closed state and draw() keeps its exact call order.
void drawTileGeometrySection(const ApplicationDebugUi::Context& context)
{
    PresentationSettings& settings = context.settings;
...
// Water surface, ripples, shoreline and the flow field.
//
// Lifted out of ApplicationDebugUi::draw, which was 854 lines - seven
// independent panels and a preamble, sharing nothing but `context`. The
// CollapsingHeader call stays inside the function so the panel keeps its own
// open/closed state and draw() keeps its exact call order.
void drawWaterSection(const ApplicationDebugUi::Context& context)
{
    PresentationSettings& settings = context.settings;
...
// Mixer levels and the device the audio system opened.
//
// Lifted out of ApplicationDebugUi::draw, which was 854 lines - seven
// independent panels and a preamble, sharing nothing but `context`. The
// CollapsingHeader call stays inside the function so the panel keeps its own
// open/closed state and draw() keeps its exact call order.
void drawAudioSection(const ApplicationDebugUi::Context& context)
{
...
// Tonemapping curve and exposure.
//
// Lifted out of ApplicationDebugUi::draw, which was 854 lines - seven
// independent panels and a preamble, sharing nothing but `context`. The
// CollapsingHeader call stays inside the function so the panel keeps its own
// open/closed state and draw() keeps its exact call order.
void drawOutputSection(const ApplicationDebugUi::Context& context)
{
    PresentationSettings& settings = context.settings;
...
// Sun, ambient, shadows and ambient occlusion.
//
// Lifted out of ApplicationDebugUi::draw, which was 854 lines - seven
// independent panels and a preamble, sharing nothing but `context`. The
// CollapsingHeader call stays inside the function so the panel keeps its own
// open/closed state and draw() keeps its exact call order.
void drawLightingSection(const ApplicationDebugUi::Context& context)
{
    PresentationSettings& settings = context.settings;
...
// Everything the renderer counted this frame, and the GPU phase timings.
//
// Lifted out of ApplicationDebugUi::draw, which was 854 lines - seven
// independent panels and a preamble, sharing nothing but `context`. The
// CollapsingHeader call stays inside the function so the panel keeps its own
    ...
    const Context& context) const
{
    ...
    drawTileGridSection(context);
    drawTileGeometrySection(context);
    drawWaterSection(context);
    drawAudioSection(context);
    drawOutputSection(context);
    drawLightingSection(context);
    drawRenderingStatsSection(context);
#else
    (void)context;
#endif
    return result;
