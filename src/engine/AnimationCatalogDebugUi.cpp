...
// The timeline section of the catalog panel, in three pieces. Guarded the same
// way draw()'s body is: with the debug UI compiled out there is nothing here to
// call them.
#if SOKOBAN_ENABLE_DEBUG_UI

// Switches the timeline section to the event editor, seeding the name field and
// lining the preview up on the event's frame.
//
// Was a [&] lambda inside draw(), which is what a 324-line function does to a
// piece of behaviour that deserves a name.
void AnimationCatalogDebugUi::openEventEditor(
    const AnimationCatalog::TimelineEvent* event,
    const TimelineContext& timeline)
{
    AnimationCatalogEditor& editor = timeline.editor;
    const AssetManifest& manifest = timeline.manifest;
    AnimationPreviewDebugUi& preview = timeline.preview;
    VulkanRenderer& renderer = timeline.renderer;
    const AnimationCatalog& catalog = timeline.catalog;
    const AnimationUse selectedUse = timeline.selectedUse;
    const RenderAnimation selectedAnimation =
        timeline.selectedAnimation;
    bool& changed = timeline.changed;
...
// The events on the selected use, and the start-gate controls under them.
void AnimationCatalogDebugUi::drawTimelineEventList(
    const TimelineContext& timeline,
    std::span<const AnimationUseDefinition> definitions)
{
    ...
    for (std::size_t i = 0; i < selectedEvents.size(); ++i) {
        ...
        if (ImGui::Button("Edit")) {
            openEventEditor(&event, timeline);
        }
        ImGui::PopID();
    }
    if (ImGui::Button("Add New Event")) {
        openEventEditor(nullptr, timeline);
    }
...
// The single-event editor: the name field, the add-at-cursor control, and the
// scrubbing preview it works against.
void AnimationCatalogDebugUi::drawTimelineEventEditor(
    const TimelineContext& timeline)
{
    AnimationCatalogEditor& editor = timeline.editor;
    AnimationPreviewDebugUi& preview = timeline.preview;
    ...
    VulkanRenderer& renderer)
{
    ...
    if (ImGui::CollapsingHeader(
        ...
        if (timelinePage_ == TimelinePage::EventList) {
            drawTimelineEventList(timeline, definitions);
        } else {
            drawTimelineEventEditor(timeline);
        }
    }
