...
struct RenderFrameData {
    ...
    // Snapshot of the live water tuning used by the render thread. The
    // authored defaults come from defaultWaterRendering() (WaterConfig.hpp),
    // defined out of line so this widely included header does not depend on
    // the water tuning values.
    struct WaterRendering {
        Vec4 surfaceColor {};
        float primaryRippleOpacity {};
        float secondaryRippleOpacity {};
    ...
    [[nodiscard]] static WaterRendering defaultWaterRendering();
